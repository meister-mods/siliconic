...
public class WaferScreen extends AbstractContainerScreen<WaferMenu> {
  ...
  public WaferScreen(WaferMenu menu, Inventory inventory, Component title) {
    super(menu, inventory, title); imageWidth = 224; imageHeight = 214;
  }

  @Override protected void init() { super.init(); gridX = leftPos + (imageWidth - GRID) / 2; gridY = topPos + 34; }
  ...
  @Override protected void renderBg(GuiGraphics graphics, float partialTick, int mouseX, int mouseY) {
    ...
    for (int y = 0; y < 8; y++) for (int x = 0; x < 8; x++) {
      ...
      graphics.fill(gridX + x * CELL + 1, gridY + y * CELL + 1, gridX + (x + 1) * CELL - 1, gridY + (y + 1) * CELL - 1, color);
    }
    int pin = 0xffffd85a;
  ...
  @Override protected void renderLabels(GuiGraphics graphics, int mouseX, int mouseY) {
    ...
    graphics.drawCenteredString(font, Component.translatable("screen.siliconic.wafer.hint"), imageWidth / 2, 199, 0xffaeb7c0);
  ...
  @Override public boolean mouseClicked(double mouseX, double mouseY, int button) {
    if (button == 0 && mouseX >= gridX && mouseX < gridX + GRID && mouseY >= gridY && mouseY < gridY + GRID) {
